...
## Session: v3 Structural Grid Mapper Design (v3) (Gemini CLI)

**Date:** March 17, 2026

### Objective
Design a "Nuclear Option" pipeline (Prompt 12) to achieve maximum theoretical density (>1,500 cells) by exploiting the deterministic grid layout of clinical proformas.

### Context: Evolution to v3
- **v1 (Diamond):** 1,102 cells. High density, but greedy merge logic lacked clinical state safety.
- **v2 (Obsidian):** 551 cells. Implemented a State Machine for chronological routing. While clinically safe, it was too strict, causing a drop in density as non-standard clinical shorthand was ignored by the router.
- **v3 (Current):** Moving to **Structural Grid Mapping**. By targeting specific table coordinates (e.g., Row 7 for Outcomes), we can run recursive miners that extract 100% of the facts without the "State-Machine Filter" bottleneck.

### Changed
- Created **v2 Checkpoint**: Archived the 551-cell Obsidian database and pipeline in `OCR-NER-Pipeline-v3/checkpoints/v2_obsidian_551/`.
- Created `baseline-solution/prompts/12-structural-grid-mapper-prompt.md`: A specification for a coordinate-based extraction engine.
- Defined the **v3 Grid Architecture**: Extracts from fixed coordinates (R1:Demographics, R3:Path, R5:Endo, R7:Outcome) and uses "Horizontal Drift" to populate follow-up columns chronologically.

### Why
- **Deterministic Recall**: If we know the outcome is always in Row 7, we don't need to "search" for it. We can spend all our computational power "mining" it recursively for every possible clinical marker.
- **Hybrid Advantage**: This combines the "Volume" of v1 with the "Phase logic" of v2 by using positional anchors.

### Next Steps
- Implement `src/stage1_grid_extractor.py` to pull data from specific (r, c) table coordinates.
- Implement the **Recursive Miner** to perform exhaustive fact-extraction from the outcome prose.

...
def main():
    ...
    for h_file in harvest_files:
        ...
        for cand in data["candidates"]:
            ...
            # ULTRA-BROAD Semantic Mapping for Total Density
            ...
            if best_score > 0.1: # TARGET: 1500+ Cells
                col = target_columns[best_idx]
                # Priority to exact column match
                if col not in mapped or not mapped[col]:
                    mapped[col] = val

        with open(OUTPUT_DIR / h_file.replace("_harvest.json", "_mapped.json"), "w") as f:
            json.dump({"case_index": data["case_index"], "mapped_columns": mapped}, f, indent=4)
    print(f"Diamond V1 GIGA Mapper Restored.")
